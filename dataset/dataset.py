...
from utils import set_folder_status, DatasetStatus, ProjectType
from dataset.utils import get_label_count
import shutil
import os
...
def import_dataset(data, workspace, monitored_processes, load_demo_proc_dict):
    ...
    from dataset.operate import import_dataset
...
def get_dataset_status(data, workspace):
    ...
    Args:
        ...
        'did':数据集id
    """
    from dataset.operate import get_dataset_status
    dataset_id = data['did']
    assert dataset_id in workspace.datasets, "数据集ID'{}'不存在.".format(dataset_id)
    dataset_type = workspace.datasets[dataset_id].type
    dataset_path = workspace.datasets[dataset_id].path
...
def get_dataset_details(data, workspace):
    ...
    Return:
        ...
        'test_files': 测试集文件列表
    """
    from dataset.operate import get_dataset_details
    dataset_id = data['did']
    assert dataset_id in workspace.datasets, "数据集ID'{}'不存在.".format(dataset_id)
    dataset_path = workspace.datasets[dataset_id].path
...
def split_dataset(data, workspace):
    ...
    """
    from dataset.operate import split_dataset
    from dataset.operate import get_dataset_details
    dataset_id = data['did']
    assert dataset_id in workspace.datasets, "数据集ID'{}'不存在.".format(dataset_id)
    dataset_type = workspace.datasets[dataset_id].type
