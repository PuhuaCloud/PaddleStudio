...
def list_tasks(data, workspace):
    ...
    for key in workspace.tasks:
        ...
        from project.operate import get_task_status
...
def get_default_params(data, workspace, machine_info):
    from project.train.params_v2 import get_params
...
def get_task_status(data, workspace):
    ...
    """
    from project.operate import get_task_status, get_task_max_saved_epochs
    tid = data['tid']
...
def stop_train_task(data, workspace):
    ...
    Args:
        ...
        'tid'任务id
    """
    from project.operate import stop_train_model
    tid = data['tid']
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
    path = workspace.tasks[tid].path
...
def start_prune_analysis(data, workspace, monitored_processes):
    ...
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
    task_path = workspace.tasks[tid].path
    from project.operate import prune_analysis_model
...
def get_prune_status(data, workspace):
    ...
    Args:
        ...
        'tid'任务id
    """
    from project.operate import get_prune_status
    tid = data['tid']
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
    path = workspace.tasks[tid].path
...
def stop_prune_analysis(data, workspace):
    ...
    tid = data['tid']
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
    from project.operate import stop_prune_analysis
...
def evaluate_model(data, workspace, monitored_processes):
    ...
    Return:
        None
    """
    from project.operate import evaluate_model
    tid = data['tid']
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
...
def get_evaluate_result(data, workspace):
    ...
    Return:
        包含评估指标的dict
    """
    from project.operate import get_evaluate_status
    tid = data['tid']
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
    task_path = workspace.tasks[tid].path
...
def get_predict_status(data, workspace):
    from project.operate import get_predict_status
    tid = data['tid']
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
    path = workspace.tasks[tid].path
...
def predict_test_pics(data, workspace, monitored_processes):
    from project.operate import predict_test_pics
    tid = data['tid']
...
def stop_predict_task(data, workspace):
    from project.operate import stop_predict_task
    tid = data['tid']
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
    path = workspace.tasks[tid].path
...
def export_lite_model(data, workspace):
    ...
    Args:
        ...
        'tid'任务id, 'save_dir'导出模型保存路径
    """
    from project.operate import opt_lite_model
...
def stop_export_task(data, workspace):
    ...
    Return:
        目前导出的状态.
    """
    from project.operate import stop_export_task
    tid = data['tid']
    assert tid in workspace.tasks, "任务ID'{}'不存在".format(tid)
    task_path = workspace.tasks[tid].path
