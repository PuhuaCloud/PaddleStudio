...
from project.evaluate.draw_pred_result import visualize_classified_result, visualize_detected_result, visualize_segmented_result
from project.visualize import plot_det_label, plot_insseg_label, get_color_map_list
...
def _call_paddle_prune(best_model_path, prune_analysis_path, params):
    ...
    if task_type == "classification":
        from project.prune.classification import prune
    elif task_type in ["detection", "instance_segmentation"]:
        from project.prune.detection import prune
    elif task_type == "segmentation":
        from project.prune.segmentation import prune
...
def _call_paddlex_train(task_path, params):
    ...
    dataset_path = params['dataset_path']
    if task_type == "classification":
        from project.train.classification import train
    elif task_type in ["detection", "instance_segmentation"]:
        from project.train.detection import train
    elif task_type == "segmentation":
        from project.train.segmentation import train
...
def _call_paddlex_evaluate_model(task_path,
    ...
    if task_type == "classification":
        from project.evaluate.classification import Evaluator
        evaluator = Evaluator(model_path, topk=topk)
    elif task_type == "detection":
        from project.evaluate.detection import DetEvaluator
        evaluator = DetEvaluator(
            model_path,
            score_threshold=score_thresh,
            overlap_thresh=overlap_thresh)
    elif task_type == "instance_segmentation":
        from project.evaluate.detection import InsSegEvaluator
        evaluator = InsSegEvaluator(
            model_path,
            score_threshold=score_thresh,
            overlap_thresh=overlap_thresh)
    elif task_type == "segmentation":
        from project.evaluate.segmentation import Evaluator
